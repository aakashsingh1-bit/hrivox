-- Admin override: last time users can place bets on a market (null = scrap schedule default).
...
BEGIN
  ...
  -- Close check: admin override first, else next_result_at − 30s
...
CREATE OR REPLACE FUNCTION admin_set_betting_closes_at(p_game_id uuid, p_closes_at timestamptz)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  END IF;
  UPDATE games
  SET betting_closes_at = p_closes_at
  WHERE id = p_game_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Game not found';
  END IF;
...
GRANT EXECUTE ON FUNCTION admin_set_betting_closes_at(uuid, timestamptz) TO authenticated;
